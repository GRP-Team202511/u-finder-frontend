...
import { deleteAccount, verifyDeleteWith2FA, verifyDeleteWithEmail } from '@/api/userApi'
...
// Current step: 'confirm' -> initial warning, 'verify' -> code input after API call
type Step = 'confirm' | 'verify'

const step = ref<Step>('confirm')
// 'email' or '2fa', determined by the server response
...
// Dynamic dialog title based on current step and verification method
...
// Step 1: call DELETE /auth/delete to initiate deletion and determine verification method
...
// Step 2: submit the verification code to complete account deletion
async function handleVerify() {
  ...
  try {
    ...
    // Account successfully deleted — clear local session and redirect to cover page
    ...
    if (error.response?.status === 401) {
      // Expired temp token or wrong email code
      toast.error(t('settings.account.deleteAccount.errors.invalidToken'))
    } else if (error.response?.status === 400) {
      // Invalid TOTP code or already-used backup code

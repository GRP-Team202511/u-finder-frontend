...
<template>
	<div class="grid gap-4">
		<Field>
			<FieldLabel :for="`duolingo-date-${props.index}`">{{ t('test.testDate') || 'Test date' }}</FieldLabel>
			<Input v-if="props.editable" :id="`duolingo-date-${props.index}`" type="date" v-model="props.entry.test_date" />

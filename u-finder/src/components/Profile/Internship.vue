...
const hasFilledInternships = computed(() =>
  internships.value.some((intern) =>
    ...
    <Card>
      <CardHeader class="text-left">
        <div class="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 gap-y-2">
          ...
          <div v-if="!localEditing && hasFilledInternships" class="flex justify-end">
          ...
          <form @submit="save">
            <FieldGroup>
              <template v-for="(intern, idx) in internships" :key="idx">
                ...
                <hr v-if="idx < internships.length - 1" class="my-6 border-t-2 border-muted-foreground/20" />
              </template>
            </FieldGroup>
